...
const CategoryScreen: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const data = await getAllCategories(null); // Assuming no data payload is needed
        setCategories(data?.data || []); // Ensure data is an array
        setError(null);
      } catch (err: any) {
      ...
      <View style={styles.errorContainer}>
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            setError(null);
            setLoading(true);
            fetchCategories(); // Retry fetching
          }}
        >
