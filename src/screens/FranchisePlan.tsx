...
const FranchisePlan: React.FC = () => {
  ...
  const fetchSubscriptionDetails = async (): Promise<void> => {
    try {
      ...
      if (!franchiseId) {
        console.error("Franchise ID is not available in AsyncStorage.");
        return;
      }
  ...
  const getPlanConfig = (planName: string) => {
    const configs: { [key: string]: { icon: string; color: string } } = {
      "Economy Plan": { icon: "bolt", color: "blue" },
      "Gold Plan": { icon: "star", color: "yellow" },
      "Platinum Plan": { icon: "crown", color: "purple" },
      "Free Plan": { icon: "shield-alt", color: "green" },
  ...
  const renderCurrentSubscription = () => (
    <View className="p-4 max-w-6xl mx-auto">
      {currentSubscription ? (
        <View className="bg-white p-6 rounded-lg shadow-md mb-8 border border-gray-200">
          <View className="flex-row justify-between items-start">
            <View className="flex-row items-start gap-5">
              <View className={`bg-${getPlanConfig(currentSubscription.subscriptionName).color}-100 p-3 rounded-full mt-1`}>
                <Icon5
                  name={getPlanConfig(currentSubscription.subscriptionName).icon}
                  size={20}
                  color={`#${getPlanConfig(currentSubscription.subscriptionName).color === "purple" ? "8b5cf6" : getPlanConfig(currentSubscription.subscriptionName).color}-600`}
                />
              </View>
                  ...
                  return (
                    ...
                    >
                      <View className="flex-row items-center mb-4">
                        <View
                          className={`bg-${config.color}-100 text-${config.color}-600 w-12 h-12 rounded-full items-center justify-center mr-4`}
                        >
                          <Icon5 name={config.icon} size={20} color={`#${config.color === "purple" ? "8b5cf6" : config.color}-600`} />
                        </View>
                        <View>
                      ...
                      {plan.fullFeatures && plan.fullFeatures.length > 0 && (
                        <View className="mt-3">
                          <Text className="text-lg font-semibold text-gray-800 mb-2">Terms & Conditions</Text>
                          <View className="space-y-2">
                            {plan.fullFeatures.map((details, i) => (
                              <View key={i} className="flex-row items-start">
                                <Text className="text-gray-700 text-sm mr-3">•</Text>
        ...
        <View className="bg-white p-6 rounded-lg shadow-md mb-8 border border-gray-200 flex-wrap">
  ...
  return (
    <ScrollView className="flex-1 bg-gray-50">
      <View className="bg-white px-6 py-4">
        <View className="flex-col items-left justify-between gap-3">
...
//         const subscriptionResponse = await getFranchiseSubscriptionPlan(franchiseId);
//         if (subscriptionResponse && subscriptionResponse.success && subscriptionResponse.result) {
...
//         // Fetch all plans
//         const plansResponse = await getFranchisePlans();
//         if (plansResponse && plansResponse.success && Array.isArray(plansResponse.data)) {
...
//     navigation.navigate("SubscriptionDetails", { plan });
//   };
...
//                           className="mt-4"
//                         >
//                           <Text className="text-center text-purple-600 font-medium">
//                             Full Details →
//                           </Text>
//                         </TouchableOpacity>
//                       </View>
...
//                   onPress={() => Alert.alert("Purchase", "Please contact Prnv Admin (call: 9603558369).")}
//                 >
...
//             {viewMode === "current" ? "Current Plan" : "All Plans"}
//           </Text>
//           <TouchableOpacity
